...
# Santa Bárbara tactical module — loaded after core to avoid circular imports
try:
    from santa_barbara import santa_barbara_router as _sb_router
    _SANTA_BARBARA_AVAILABLE = True
except Exception as _sb_import_err:  # pragma: no cover
    import logging as _logging
    _logging.getLogger(__name__).warning(
        f"Santa Bárbara module could not be loaded: {_sb_import_err}"
    )
    _sb_router = None
    _SANTA_BARBARA_AVAILABLE = False
...
# Santa Bárbara tactical router — registered before catch-all SPA route
if _SANTA_BARBARA_AVAILABLE and _sb_router is not None:
    app.include_router(_sb_router)
    logger.info("Santa Bárbara tactical module registered at /api/v5/santa_barbara")
